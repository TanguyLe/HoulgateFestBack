...
exports.completeShotgun = (roomId, roommatesId, callback) => {
    ...
    Shotgun.findOneAndUpdate(
        ...
        .then((shotgun) => {
            if (!shotgun) return callback(shotgunErrors.getShotgunNotFoundError(req.params.roomID));
...
exports.afterCompleteShotgun = (shotgun) => {
                    ...
                    if (err) {
                        ...
                        return callback(
                            errors.getServerError(
                                "Couldn't send email to users after shotgun " +
                                    populatedShotgun._id +
                                    " completed."
                            )
                        );
                    }
    ...
    async.waterfall([retrieveShotgun.bind(null, shotgun), sendMails], (err, result) => {
        if (err)
            return console.error(
                "-> After complete shotgun failed for shotgun with id:" + shotgun._id + "."
            );
